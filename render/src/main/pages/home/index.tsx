...
class Home extends React.PureComponent<any, IState> {
  ...
  componentDidMount(): void {
    ...
    Mousetrap.bind('left', this.renderlLast, 'keypress')
    ...
    window.electronAPI.onPositions((_event, value) => {
      ...
      this.setState(
        {
          dataSource: [...value],
          dataArray: this.hasOriginData ? [] : this.state.dataArray,
          historyIndex: Math.max(0, this.history.length - 1)
        },
        () => {
          this.history.push({
      ...
      <>
        <div className="app-header">
          <div className="button_group">
            ...
            <div className="appName">小鲸鱼</div>
            <div className="dropdown">
              <button className="dropbtn"></button>
              <div className="dropdown-content">
                <a href="#" onClick={() => window.electronAPI.openFile()}>
                  打开
                </a>
                <a href="#" onClick={() => window.electronAPI.saveFile()}>
                  保存
                </a>
                <a href="#" onClick={() => window.electronAPI.saveAs()}>
                  导出数据
                </a>
              </div>
            </div>
            <div
              ...
              onClick={this.onClickPreFare}
              onMouseDown={this.onMouseDownPreFare}
              onMouseUp={this.onMouseUpPreFare}
            ></div>
            ...
            <div
              ...
              onClick={this.onClickNextFare}
              onMouseDown={this.onMouseDownNextFare}
              onMouseUp={this.onMouseUpNextFare}
            ></div>
            <input
              type="range"
              min={0}
          ...
          <div className="home">
            <div className="position">
              <div className="desc">
                ...
                <p>
                  定位精度:{' '}
                  {curPosition && curPosition[5] && cName[+curPosition[5]]}
                </p>
              </div>
  ...
  private renderLineChartA(positions) {
    ...
    this.lineChartA.on('moverover', (params) => {
      const {
        selected,
    ...
    this.lineChartA.on('datazoom', ({ batch }) => {
      ...
      this.setState(
        ...
        () => {
          this.lineChartB.dispatchAction({
            type: 'dataZoom',
            startValue: +this.state.startValue.toFixed(0),
            endValue: +this.state.endValue.toFixed(0)
          })
        }
      )
    })
  ...
  private renderScatterChartA(positions) {
    ...
    this.zender1 = this.scatterChartA.getZr()
    ...
    this.scatterChartA.on('selectchanged', (params) => {
      ...
      const data1 = this.state.dataSource[dataIndex]
      const B = translate(data1.split(',')[1])
      const L = translate(data1.split(',')[2])
      const H = +data1.split(',')[4]
      const res = wgs(B, L, H)
      const point = this.scatterChartA.convertToPixel('grid', [res.X, res.Y])
      this.clearTriangle()
      this.drawTriangle(point[0], point[1], 5)
      let startValue = this.state.startValue
      let endValue = this.state.endValue
      const range = endValue - startValue
      if (dataIndex < startValue || dataIndex > endValue) {
        startValue = Math.max(dataIndex - +(range / 2).toFixed(0), 0)
        endValue = Math.min(startValue + range, this.state.dataSource.length)
      }
      this.setState(
        {
    ...
    this.scatterChartA.on('datazoom', ({ batch }) => {
      ...
      const data1 = this.state.dataSource[this.state.scatterChartAelectedIndex]
      const B = translate(data1.split(',')[1])
      const L = translate(data1.split(',')[2])
      const H = +data1.split(',')[4]
      const res = wgs(B, L, H)
    ...
    this.setState(
      ...
      () => {
        this.lineChartA.setOption({
          ...
          tooltip: {
            show: !!this.state.mode ? false : true
          }
        })
        this.lineChartB.setOption({
          dataZoom: [
        ...
        dataArray.forEach((data, index) => {
          if (index >= startIndex && index <= endIndex) {
            ...
            if (lastPosition && nextPosition) {
              for (
                let i = lastPosition.index + 1;
                i < nextPosition.index;
                i++
              ) {
                dataArray[i] =
                  nextPosition.value -
  ...
  draw = () => {
    ...
    this.lineChartB.setOption({
      ...
      series: [
        ...
        {
          name: '新高度',
          data: newPositionY.map((position) => position * -1)
        }
      ]
    })
  ...
  setselected = (point) => {
    const selectedPoint = this.lineChartA.convertFromPixel('grid', [
      point.x,
      point.y
    ])
    if (
      selectedPoint[0] < this.state.startValue ||
      selectedPoint[0] > this.state.endValue
    ) {
      return
    }
    this.setState(
