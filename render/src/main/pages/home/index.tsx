...
class Home extends React.PureComponent<any, IState> {
  ...
  componentDidMount(): void {
    ...
    window.electronAPI.sendHistoryToRender((_event, value) => {
      ...
      this.setState({
        dataArray: _.isEmpty(lastDatarray) ? this.state.dataArray : lastDatarray,
        dataSource: _.isEmpty(lastDataSource) ? this.state.dataSource : lastDataSource,
      ...
      <>
        <div className="app-header">
          <div className="button_group">
            <div className="left_button" onClick={this.onClickPreFare}>
            </div>
            {this.state.mode !== 'play' && <div className="play_button" onClick={this.onClickPlay}>
            </div>}
            {this.state.mode === 'play' && <div className="pause_button" onClick={this.onClickPause}></div>}
            <div className="right_button" onClick={this.onClickNextFare}>
            </div>
            <div className={this.state.mode === 'delete' ? "del_button_selected" : "del_button"} onClick={this.onClickDelete}>
            </div>
          </div>
          <div className="app-header-right">
          </div>
          <div className="toolbox">
            <div className={this.state.mode === 'pencil' ? "pencil_selected" : "pencil"} onClick={this.onClickPencil}>
            </div>
            <div onClick={this.onClickRestore} className="restore">
            </div>
            <div onClick={this.onClickPre} className="pre_step">
            </div>
            <div onClick={this.onClickNext} className="next_step">
            </div>
          </div>
        </div>
        <div className="app-content">
          <div className="home">
            <div className="position">
              <div className="desc">
                <p>当前光标位置:</p> <p>X: {curPosition && curPosition[1]}</p> <p>Y:{curPosition && curPosition[2]}</p> <p>深度: {curPosition && curPosition[3]} ({
  ...
  private renderLineChartA(positions) {
    ...
    this.lineChartA.on('selectchanged', (params) => {
      const { selected, fromActionPayload: { seriesIndex } } = params
      const data = selected
      if (!selected || !seriesIndex) {
        return
      }
      let dataIndex = 0
      data.forEach(d => { if (+d.seriesIndex === +seriesIndex) { dataIndex = d.dataIndex[0] } })
  ...
  private renderLineChartB(positions) {
    ...
    newImageObj.src = 'data:image/png;base64,'
  ...
  // 键盘事件
  renderlLast = () => {
    const lastSelectedIndex = Math.max(this.state.lineChartASelectedIndex - 1, 0)
  ...
  renderNext = () => {
    const lastSelectedIndex = Math.min(this.state.lineChartASelectedIndex + 1, this.state.dataSource.length)
    let lastEndValue = this.state.endValue
    let lastStartValue = this.state.startValue
    if (lastSelectedIndex > lastEndValue) {
      lastEndValue = lastEndValue + 1
  ...
  onClickPre = (e) => {
    ...
    const { dataArray, dataSource } = this.history[this.state.historyIndex] || { dataArray: [], dataSource: [...this.originData] }
    this.setState({
  ...
  onClickNext = (e) => {
    e.stopPropagation()
    this.setState({
      historyIndex: Math.min(this.history.length - 1, this.state.historyIndex + 1)
    }, () => {
  ...
  // 下一桢
  onClickNextFare = (e) => {
    let lastEndValue = Math.min(this.state.endValue + 1, this.state.dataSource.length)
  ...
  // 上一桢
  onClickPreFare = (e) => {
    let lastStartValue = Math.min(this.state.startValue - 1, this.state.dataSource.length)
        ...
        this.pencilTmp.forEach(position => {
          ...
          if (lastPosition) {
            for (let i = lastPosition.x + 1; i < index; i++) {
              dataArray[i] = value - (((index - i) / (index - lastPosition.x)) * (value - lastPosition.y))
            }
          }
          if (nextPosition) {
            for (let i = index + 1; i <= nextPosition.x; i++) {
              dataArray[i] = value - (((index - i) / (index - nextPosition.x)) * (value - nextPosition.y))
            }
          }
        })
        break
  ...
  draw = () => {
    ...
    this.state.dataSource.forEach((item, index) => {
      ...
      minY = isUndefined(minY) ? res.Y : minY > res.Y ? res.Y : minY
      maxY = isUndefined(maxY) ? res.Y : maxY < res.Y ? res.Y : maxY
      minPositionH = isUndefined(minPositionH) ? Math.min(this.state.dataArray[index] || 0, level) : minPositionH > Math.min(this.state.dataArray[index] || 0, level) ? Math.min(this.state.dataArray[index] || 0, level) : minPositionH
      maxPositionH = isUndefined(maxPositionH) ? Math.min(this.state.dataArray[index] || 0, level) : maxPositionH < Math.max(this.state.dataArray[index] || 0, level) ? Math.max(this.state.dataArray[index] || 0, level) : maxPositionH
    })
